...
public class KBLowerEnvironment extends KBEnvironment {
	...
	public void buildEnvironment(NerdConfig conf, boolean overwrite) throws Exception {
		...
		File wordEmbeddingsFile = getWordEmbeddingsDataFile(dataDirectory);
		File entityEmbeddingsFile = getEntityEmbeddingsDataFile(dataDirectory);
		...
		File[] files = dataDirectory.listFiles(new FilenameFilter() {
			public boolean accept(File dir, String name) {
				return name.endsWith("-pages-articles.xml") || name.endsWith("-pages-articles.xml.bz2") || name.endsWith("-pages-articles.xml.gz") || 
						name.endsWith("-pages-articles-multistream.xml") || name.endsWith("-pages-articles-multistream.xml.bz2") || 
						name.endsWith("-pages-articles-multistream.xml.gz") ;
			}
		});
	...
	/**
	 *  Two types of embeddings are supported:
	 *	- embeddings generated by entity-fishing tooling, in two separate files
	 *	  one for word embeddings (normally ) and one for entity embeddings
	 *  - wikipedia2vec word and entity embeddings (https://wikipedia2vec.github.io/wikipedia2vec/)
	 *	  which are available in a single file
	 */
