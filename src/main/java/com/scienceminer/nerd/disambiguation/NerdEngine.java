...
public class NerdEngine {
	...
	public Map<NerdEntity, List<NerdCandidate>> generateCandidatesSimple(List<NerdEntity> entities, String lang) {
		...
		for(NerdEntity entity : entities) {
			...
			// for NER type generated original conll 2003 type
			String nerConll_type = entity.getNerConllType();
...
//System.out.println("check categories for " + sense.getId());							
						...
						// for ner generated by conll 2003 (filling the candidates NE type with the entity NE type)
						candidate.setNerConllType(nerConll_type);
						...
						*/
						...
						for (Entity entityResult : resultNEExtractText) {
							...
							if (mention.getType() != null) {
								candidate.setNerGrobidType(mention.getType().toString());
							} else{
								candidate.setNerGrobidType("UNKNOWN");
							}
						}

						// for nerKid_type
						String nerKid_type = UpperKnowledgeBase.getInstance().getPredictedClassByWikidataId(candidate.getWikidataId());
						if (nerKid_type != null) {
							nerKid_type = upperKnowledgeBase.getInstance().getPredictedClassByWikidataId(candidate.getWikidataId());
						} else {
							nerKid_type = "OTHER";
						}
						candidate.setNerdKidType(nerKid_type);
				...
				for(int p=0; p<bestLabels.size() && s==0; p++) {
					...
					if ((senses != null) && (senses.length > 0)) {
						for(int i=0; i<senses.length; i++) {
							...
							// for ner generated by conll 2003
							candidate.setNerConllType(nerConll_type);
...
//			    LOGGER.debug("Candidate: " +  candidate.toString());
				...
				try {
					...
					// for conll 2003 type
					String nerConll_type = "MISC";
					if (candidate.getNerConllType() != null){
						nerConll_type = candidate.getNerConllType();
					}

					// for nerGrobid_type
					String nerGrobid_type = "UNKNOWN"; // undefined entity
					if (candidate.getNerGrobidType() != null)
						nerGrobid_type = candidate.getNerGrobidType();

					// for nerKid_type
					String nerKid_type = "OTHER"; // undefined entity
					if (candidate.getNerdKidType() != null)
						nerKid_type = candidate.getNerdKidType();

					score = ranker.getProbability(commonness, related, quality,
							bestCaseContext, embeddingsSimilarity, wikidataId, wikidataP31Id, nerConll_type, nerKid_type, nerGrobid_type);
	...
	private void rank(List<NerdCandidate> candidates, String rawTerm, List<WeightedTerm> terms,
		...
		for(NerdCandidate candidate : candidates) {
			...
			try {
				...
				String nerGrobid_type = "UNKNOWN";
				if(candidate.getNerGrobidType() != null){
					nerGrobid_type = candidate.getNerGrobidType();
				}

				// for NER type generated by nerdKid
				String nerKid_type = "OTHER";
				if(candidate.getNerdKidType() != null){
					nerKid_type = candidate.getNerdKidType();
				}
...
System.out.println(acronym.getRawName() + " / " + base.getRawName());
	...
	private void updateEntity(NerdEntity toBeUpDated, NerdEntity best) {
		...
		toBeUpDated.setNerConllType(best.getNerConllType());
		toBeUpDated.setNerGrobidType(best.getNerGrobidType());
        toBeUpDated.setNerdKidType(best.getNerdKidType());
