...
public class NEDCorpusEvaluation {
    ...
    public String recognizeLanguge(File docFile) {
    ...
    public String eval(String corpus) {
        ...
        for (int i = 0; i < docs.getLength(); i++) {
            ...
            String langId = recognizeLanguge(docFile);
            ...
            for (int j = 0; j < annotations.getLength(); j++) {
				...
				/* the label of empty wikiName can be in 'null' or 'NIL'
                */
                ...
                // re-initialisation of ranker model due to the change of document language
                try {
                    UpperKnowledgeBase.getInstance();
                } catch (Exception e) {
                    throw new NerdResourceException("Error instanciating the knowledge base. ", e);
                ...
                wikipedia = UpperKnowledgeBase.getInstance().getWikipediaConf(langId);
                try {
                    ranker = new NerdRanker(wikipedia);
                } catch (Exception e) {
                    throw new NerdResourceException("Error when opening the relatedness model", e);
            ...
            try {
                ...
                Map<NerdEntity, List<NerdCandidate>> candidates =
                        engine.generateCandidatesMultiple(entities, langId);
...
System.out.println("--");
        ...
        } // end of looping for each document in xml reference file
